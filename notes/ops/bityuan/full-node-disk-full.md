...
- **放大器：`dbCache` 是"三合一"的，取值太小**。`common/db/go_level_db.go:73-86` 把**同一个 `dbCache` 同时喂给三个 leveldb 参数**：`OpenFilesCacheCapacity = dbCache`（个）、`BlockCacheCapacity = dbCache/2` MiB、`WriteBuffer = dbCache/4` MiB。配置里 `dbCache=64` ⇒ 只有 **64 个打开槽**（实测进程只开了 69 个 `.ldb` fd，**已饱和**）、块缓存只有 **32 MiB**（面对 726 GiB 的库 ≈ 零命中）。而版本里有 **40~50 万个文件**，只给 64 个槽，每次 compaction 只能小步搬运（实测 253 秒内 914 次 compaction、约 24 张表/秒）。
...
**① 先改配置**（必须在启动前改，重启后才生效）：`[blockchain] dbCache` 由 **64 → 1024**。

> **为什么是 1024**：`dbCache` 一个整数同时当三个参数用（`common/db/go_level_db.go:73-86`）——打开文件句柄数 `= dbCache`（个）、块缓存 `= dbCache/2` MiB、写缓冲 `= dbCache/4` MiB。三者最优值差一个数量级，只能折中：
>
> | | `64`（现） | **`1024`（建议）** | `2048` |
> |---|---|---|---|
> | 打开文件句柄 | 64 | 1024 | 2048 |
> | 块缓存 | 32 MiB | 512 MiB | 1 GiB |
> | 写缓冲（内部用 2 个） | 16 MiB | 256 MiB | 512 MiB |
> | 新增内存 | — | **≈ +1 GiB** | ≈ +2 GiB |
>
> 句柄数与块缓存都想要大，但**写缓冲反而想要小**（越大 → 单次 flush 延迟尖峰、崩溃后 WAL 重放越长），所以**不是越大越好**。1024 是拐点，**不建议超过 2048**。
> **保守可取 512**（+0.5 GiB）：该机 9/1 曾被 OOM 杀过（RSS 冲到约 51 GB，机器 60 GB），担心内存就选它。
> fd limit 不是瓶颈（实测进程上限 524288，当前只用了几十个）。
...
- `dbCache`（**Step 1** 的 64 → **1024**）**至今仍未应用**——注意这**不影响回收**（见上文生产验证），它是**性能/防复发项**，建议补上
